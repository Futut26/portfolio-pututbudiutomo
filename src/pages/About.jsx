  ...
  return (
    <section className="max-container">
      <h1 className="head-text">
        Hello, I'm <span className="blue-gradient_text font-semibold drop-shadow"> Putut Budiutomo</span> 👋
      </h1>
      ...
      <div className="py-10 flex flex-col">
        ...
        <div className="mt-16 grid grid-cols-3 gap-4 md:grid-cols-7 ">
          {skills.map((skill) => (
            <div className="block-container w-20 h-20" key={skill.name}>
              <div className="btn-back rounded-xl" />
              <div className="btn-front rounded-xl flex flex-col justify-center items-center">
                <img src={skill.imageUrl} alt={skill.name} className="w-1/2 h-1/2 object-contain" />
                {/* create label name for image */}
                <p className="text-center text-black-500/50 font-medium text-sm">{skill.name}</p>
              </div>
            </div>
          ))}
        </div>
